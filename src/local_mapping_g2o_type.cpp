...
*/
    typedef g2o::BlockSolver<g2o::BlockSolverTraits<6, 3>> BlockSolverType;
    g2o::LinearSolver<BlockSolverType::PoseMatrixType> *linearSolver =
        new g2o::LinearSolverCholmod<BlockSolverType::PoseMatrixType>();
    dynamic_cast<g2o::LinearSolverCholmod<BlockSolverType::PoseMatrixType> *>(linearSolver)->setBlockOrdering(true);

    //矩阵块求解器
    BlockSolverType *solver_ptr = new BlockSolverType(std::unique_ptr<BlockSolverType::LinearSolverType>(linearSolver));
...
void LocalMapping_g2o::buildGraph(g2o::SparseOptimizer &o)
{
    ...
    c->setEstimate(g2o::SE3Quat(_frames_BA[0]->_T_c2w.rotationMatrix(), _frames_BA[0]->_T_c2w.translation()));
    o.addVertex(c);
...
*/
    ...
    /*    for (int i = 0; i < _currFrame->_map_points.size(); i++)
    {
        ...
        p->setId(i + 1);
        _currFrame->_map_points[i]->_id_g2o = i + 1;
...
*/
    ...
    for (auto mp : _mps_BA)
    {
        ...
        p->setFixed(true); //全部点都固定,只优化相机位姿来减少重投影误差  只有设定成fixed,结果才正常, 不过这不是基本等于没有优化么?    BUG
        p->setMarginalized(true);
        o.addVertex(p);
    }

    g2o::CameraParameters *camera = new g2o::CameraParameters(718.856, Eigen::Vector2d(607.1928, 185.2157), 0);
    camera->setId(0);
    o.addParameter(camera);
    ...
    for (int i = 0; i < frames_size - 1; ++i)
    {
        ...
        e->setId(edge_index);
        edge_index++;
        e->setVertex(0, dynamic_cast<g2o::VertexSE3Expmap *>(o.vertex(i)));
        e->setVertex(1, dynamic_cast<g2o::VertexSE3Expmap *>(o.vertex(i + 1)));
        
        Sophus::SE3d trans = _frames_BA[i]->_T_c2w * _frames_BA[i+1]->_T_w2c;
        e->setMeasurement(g2o::SE3Quat(trans.rotationMatrix(), trans.translation()));
    ...
    for (int i = 0; i < frames_size; ++i)
    {
        ...
        for (auto mp : mps)
        {
            //if (mp->observations > 1)
            {
                ...
                edge_index++;
                e->setVertex(0, dynamic_cast<g2o::VertexSBAPointXYZ *>(o.vertex(mp->_id_g2o)));
                e->setVertex(1, dynamic_cast<g2o::VertexSE3Expmap *>(o.vertex(f->_id_g2o)));
